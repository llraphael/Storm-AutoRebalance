...
public class DynamicScheduler {
	...
	public static Map<String, Double> getCapacity(
			Map<String, Map<String, String>> boltsInfo, int timeWindow) {
		Map<String, Double> capacityResult = new HashMap<String, Double>();
		int activateTime = Integer.parseInt(boltsInfo.get("static_time").get(
				"up_time"));

		int timeBase = activateTime > timeWindow ? timeWindow : activateTime;
		boltsInfo.remove("static_time");

		for (Map.Entry<String, Map<String, String>> entry1 : boltsInfo
				.entrySet()) {
			...
			// Calculate the capacity for current bolt (actually for each
			// executor)
			if (numOfExecuted == null || avgExecuteLatency == null) {
				System.out
				.println("Bolts info incomplete. Capacity can not be calculated!");
				return capacityResult;
			} else {
				int numOfExecutors = Integer.parseInt(currentBolt
						.get("num_of_executors"));
				// Capacity formula
				Double cap = numOfExecuted * avgExecuteLatency
						/ (timeBase * 1000) / numOfExecutors;
		...
		for (Map.Entry<String, Double> entry : capacityResult.entrySet()) {
			System.out.println("Bolt Name: " + entry.getKey() + ". Capacity: "
					+ entry.getValue());
		}
	...
	public static void main(String[] args) throws InterruptedException {
		...
		ScheduledExecutorService timeManager = Executors
				.newScheduledThreadPool(1);
		AutoRebalancer balancer = new AutoRebalancer("matlab");

		// Time interval between rebalance should not be too short, which would
		// cause
		// the failure of reading executor status. A minimum value should be
		// 120s.
		ScheduledFuture<?> taskHandler = timeManager.scheduleWithFixedDelay(
				balancer, 10, 20, TimeUnit.SECONDS);
...
class AutoRebalancer implements Runnable {
	...
	public void run() {
		...
		TopologyComponentInfo componentsInfo = new TopologyComponentInfo(
				nimbusIP);
		...
		// Get bolts Info
		Map<String, Map<String, String>> boltsInfo = componentsInfo
				.getAllBoltsInfo(topologyName);

		if (boltsInfo.isEmpty()) {
			System.out
			.println("Bolts information are not available this time! Will check later.");
			return;
		}

		// Get capacity
		Map<String, Double> capacity = DynamicScheduler.getCapacity(boltsInfo,
				timeWindow);

		// Capacity Threshold
		double upThreshold = 0.9, lowThreshold = 0.2;

		// The new decided number of executors that should be assigned each
		// bolt.
		Map<String, Integer> newAssignment = new HashMap<String, Integer>();

		// Indicate that if there is actually a change in the number of newly
		// decided executor
		// numbers for each bolt.
		int changeNumber = 0;

		// Check each bolt's capacity and determine the new number of executors
		// for each bolt
		for (Map.Entry<String, Double> entry : capacity.entrySet()) {
			Integer oldNumber = Integer.parseInt(boltsInfo.get(entry.getKey())
					.get("num_of_executors"));
		...
		if (changeNumber > 0) {
			...
			try {
				...
				.println("Topology "
						+ topologyName
						+ " is being rebalanced, new number of executors for each bolt:");
				for (Map.Entry<String, Integer> entry : newAssignment
						.entrySet()) {
					System.out
			...
			System.out.println("There is no need to rebalance \""
					+ topologyName + "\"");
		}
